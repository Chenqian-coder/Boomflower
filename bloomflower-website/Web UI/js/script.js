...
var load_flickr = function(){
    $('.widgetFlickr').each(function(){
        ...
        $.getJSON("/services/feeds/photos_public.gne?lang=en-us&format=json&id="+stream_userid+"&jsoncallback=?", function(stream_feed){
...
var load_twitter = function(){
    var linkify = function(text){
        ...
        text = text.replace(/(^|)@(\w+)/gi, function (s) {
            return '<a class="textcolor7" ' + s + '">' + s + '</a>';
        });
        text = text.replace(/(^|)#(\w+)/gi, function (s) {
            return '<a class="textcolor7"/search?q=' + s.replace(/#/,'%23') + '">' + s + '</a>';
        });
        return text;
    }
    $('.widgetTwitter').each(function(){
        ...
        t.append(t_loading);
        $.getJSON("/1/statuses/user_timeline/"+t_user+".json?callback=?", function(t_tweets){
            t.empty();
